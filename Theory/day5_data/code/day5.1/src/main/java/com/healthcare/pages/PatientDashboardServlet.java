			...
			if (patient != null) {
				...
				allUpcomingAppoints.forEach(dto -> {
					pw.print("<h5> "+dto.getAppointmentId()+" "+dto.getAppointmentTS()+" "+dto.getDocName()+"</h5>"
							+  "<h5> <a href='appointments?action=cancel&id=" + dto.getAppointmentId() + "&pid="+patient.getId()
							+ "'>Cancel</a> ");
					
//					pw.print("<h5> <a href='appointments?action=cancel&id=" + dto.getAppointmentId() + "&pid="+patient.getId()
//							+ "'>Cancel</a> ");
